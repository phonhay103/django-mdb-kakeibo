...
class PaymentCreate(LoginRequiredMixin, generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        payment.save()
        msg = plugins.success_message_for_item(
            "Register", "Payment", payment.date, payment.category, payment.amount
        )
...
class IncomeCreate(LoginRequiredMixin, generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        income.save()
        msg = plugins.success_message_for_item(
            "Register", "Income", income.date, income.category, income.amount
        )
        messages.info(self.request, msg)
        return redirect(self.get_success_url())
...
class AssetCreate(LoginRequiredMixin, generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        asset.save()
        msg = plugins.success_message_for_item(
            "Register", "Asset", asset.date, asset.category, asset.amount
        )
        messages.info(self.request, msg)
        return redirect(self.get_success_url())
...
class PaymentDelete(LoginRequiredMixin, generic.DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        payment.delete()
        msg = plugins.success_message_for_item(
            "Delete", "Payment", payment.date, payment.category, payment.amount
        )
        messages.info(self.request, msg)
        return redirect(self.get_success_url())
...
class IncomeDelete(LoginRequiredMixin, generic.DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        income.delete()
        msg = plugins.success_message_for_item(
            "Delete", "Income", income.date, income.category, income.amount
        )
        messages.info(self.request, msg)
        return redirect(self.get_success_url())
...
class AssetDelete(LoginRequiredMixin, generic.DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        asset.delete()
        msg = plugins.success_message_for_item(
            "Delete", "Asset", asset.date, asset.category, asset.amount
        )
        messages.info(self.request, msg)
...
class MonthlyBalance(
    LoginRequiredMixin, plugins.MonthlyBalanceMixin, generic.TemplateView
):
    """月間収支ページ"""
...
class TransitionView(
    LoginRequiredMixin, plugins.BalanceTransitionMixin, generic.TemplateView
):
    """月毎の収支推移ページ"""
